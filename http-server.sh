...
while true; do
  ...
  if command -v nc.traditional >/dev/null 2>&1; then
    ...
    # 🔥 fallback universal
    while true; do
      nc -l -p "$PORTA" | while read line; do
        echo "$line" | "$PROGRAMA"
      done
    done
  fi
...
# 🔥 IP REAL (NO confiar en cliente)
USRIP=$(echo "$SSH_CLIENT" | awk '{print $1}')
[[ -z "$USRIP" ]] && USRIP=$(echo "$REMOTE_ADDR")
[[ -z "$USRIP" ]] && USRIP="0.0.0.0"
...
if [[ -e $PERM ]]; then
  ...
  IP_SAVED=$(cat "$PERM" | tr -d '\r\n ')

  if [[ "$IP_SAVED" != "$USRIP" ]]; then

    log="/etc/gerar-sh-log"

    echo "USUARIO: $(cat ${FILE2}.name 2>/dev/null) IP FIJA: $IP_SAVED USO IP: $USRIP" >> $log
    echo "IP DIFERENTE - ACCESO BLOQUEADO" >> $log
    echo "--------------------------------------------------------------------" >> $log

    # 🔥 NO BORRA KEY (respeta tu lógica nueva)
    exit 0
  fi
