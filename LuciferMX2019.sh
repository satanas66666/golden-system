...
# 🔧 FIX: fun_bar compatible (MISMA FORMA DE USO)
fun_bar () {
comando="$1"
(
...
# 🔧 FIX: detección paquetes más estable
...
# 🔧 FIX IP sin romper lógica
fun_ip () {
MIP=$(hostname -I | awk '{print $1}')
MIP2=$(wget -qO- ipv4.icanhazip.com)
[[ "$MIP" != "$MIP2" ]] && IP="$MIP2" || IP="$MIP"
}
# Seguna Parte de Arreglar
inst_components () {
...
# apache (misma lógica, más compatible)
...
echo -e " menu / adm" && msg -verm "$(source trans -b es:${id} "INICIE SESION CUANDO SE CIERRE ESTA TERMINAL"|sed -e 's/[^a-z -]//ig')"
...
error_fun () {
msg -bar2 && msg -verm "$(source trans -b es:${id} "Esta clave era de otro servidor, por lo tanto fue excluida"|sed -e 's/[^a-z -]//ig') " && msg -bar2
...
IP=$(ofus "$Key" | grep -oE '[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+') && echo "$IP" > /usr/bin/vendor_code
...
if [[ -e $HOME/lista-arq ]] && [[ ! $(cat $HOME/lista-arq|grep "KEY INVALIDA!") ]]; then
   ...
   listaarqs="$(locate lista-arq 2>/dev/null | head -1)" && [[ -e ${listaarqs} ]] && rm $listaarqs   
